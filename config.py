...
class Settings:
    ...
    starting_capital: float = _float("STARTING_CAPITAL", 10000)
    max_daily_loss_pct: float = _float("MAX_DAILY_PORTFOLIO_LOSS_PCT", 5)
    max_daily_loss_dollars: float = _float("MAX_DAILY_LOSS_DOLLARS", 500)
    max_position_weight_pct: float = _float("MAX_POSITION_WEIGHT_PCT", 20)
    max_new_position_weight_pct: float = _float("MAX_NEW_POSITION_WEIGHT_PCT", 5)
    max_daily_deployment_pct: float = _float("MAX_DAILY_DEPLOYMENT_PCT", 10)
